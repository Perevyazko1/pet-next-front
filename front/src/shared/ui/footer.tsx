...
const inputClassNames = {
  ...
  innerWrapper: 'pb-0',
  input: '!text-black !font-normal placeholder:text-[#121212B2] placeholder:text-sm',
};
...
export const Footer = () => {
  ...
  const handleSubmit = () => {
    ...
    if (form.email && !isValidEmail(form.email)) next.email = 'Некорректный email';
  ...
  return (
    <footer className="mt-10 border-t border-accent bg-white px-5 py-8 md:px-12">
      <div className="mx-auto flex max-w-[1518px] flex-col items-center justify-between gap-6 sm1:flex-row sm1:gap-4">
        <span className="text-xl font-bold text-primary">{texts.paws}</span>

        <div className="flex flex-col items-center gap-1 text-sm text-gray-500 sm1:items-start">
          <span>{texts.numberPhone}</span>
          <span>{texts.address}</span>
          <span>
            {texts.openingDays}&nbsp;{texts.openingHours}
          </span>
        </div>
